...
function DeletedShipper() {
  ...
  useEffect(() => {
    const fetchShippers = async () => {
      try {
        const response = await AxiosInstance.get("shippers/listShipperIsDeleted");
        console.log(response);
        // Kiểm tra và xử lý dữ liệu từ API
        if (response.data.deletedShipper
          && Array.isArray(response.data.deletedShipper
          )) {
          setShippers(response.data.deletedShipper
          );
        } else {
